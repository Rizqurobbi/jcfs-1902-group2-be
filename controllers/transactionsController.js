...
module.exports = {
    ...
    checkout: async (req, res) => {
        try {
            if (req.dataUser.role == 'User') {
                let insertTransactions = await dbQuery(`INSERT INTO transactions values (null,${req.dataUser.iduser},${db.escape(req.body.idaddress)},7,${db.escape(req.body.invoice)},${db.escape(req.body.date)},${db.escape(req.body.total_price)},${db.escape(req.body.shipping)},${db.escape(req.body.total_payment)},${db.escape(req.body.notes)},'From cart user',null)`)
    ...
    getAllTransactionsAdmin: async (req, res) => {
        try {
            let getTransactions = await dbQuery(`SELECT t.*, u.username, s.status, a.* from transactions t JOIN status s on t.idstatus = s.idstatus JOIN users u ON u.iduser = t.iduser JOIN address a on a.idaddress = t.idaddress ${req.query.invoice ? `WHERE t.invoice LIKE '%${req.query.invoice}%' or u.username LIKE '%${req.query.invoice}%'` : ''}${req.query.start_date && req.query.end_date ? `and Date(date) between '${req.query.start_date}' and '${req.query.end_date}'` : ''}`)
    ...
    getPastTransactionsAdmin: async (req, res) => {
        try {
            let getTransactions = await dbQuery(`SELECT t.*, u.username, s.status, a.* from transactions t JOIN status s on t.idstatus = s.idstatus JOIN users u ON u.iduser = t.iduser JOIN address a on a.idaddress = t.idaddress where (t.idstatus = 5 or t.idstatus = 6) ${req.query.invoice ? `and t.invoice LIKE '%${req.query.invoice}%' or u.username LIKE '%${req.query.invoice}%'` : ''}${req.query.start_date && req.query.end_date ? `and Date(date) between '${req.query.start_date}' and '${req.query.end_date}'` : ''}`)
            let getDetail = await dbQuery(`SELECT t.idtransaction, t.iduser, t.invoice, t.date, t.shipping, t.total_payment, t.notes,t.description, d.*, i.url, p.nama, p.harga, un.satuan from detail_transactions d
            JOIN products p ON p.idproduct = d.idproduct 
            JOIN images i on p.idproduct = i.idproduct
            JOIN transactions t on t.idtransaction = d.idtransaction
    ...
    getOngoingTransactionsAdmin: async (req, res) => {
        try {
            let getTransactions = await dbQuery(`SELECT t.*, u.username, s.status, a.* from transactions t JOIN status s on t.idstatus = s.idstatus
            JOIN users u ON u.iduser = t.iduser
            JOIN address a on a.idaddress = t.idaddress
            where (t.idstatus = 4 or t.idstatus = 7 or t.idstatus = 8) ${req.query.invoice ? `and t.invoice LIKE '%${req.query.invoice}%' or u.username LIKE '%${req.query.invoice}%'` : ''}${req.query.start_date && req.query.end_date ? `and Date(date) between '${req.query.start_date}' and '${req.query.end_date}'` : ''}`)
            let getDetail = await dbQuery(`SELECT t.idtransaction, t.iduser, t.invoice, t.date, t.shipping, t.total_payment, t.notes,t.description, d.*, i.url, p.nama, p.harga, un.satuan from detail_transactions d
            JOIN products p ON p.idproduct = d.idproduct 
            JOIN images i on p.idproduct = i.idproduct
            JOIN transactions t on t.idtransaction = d.idtransaction
